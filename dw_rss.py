...
INDEX_URL = 'https://www.dw.com/zh/'
OUTPUT_FILE = 'dw_feed.xml'
...
def extract_article(html):
    ...
    # 德国之声的正文容器
    article = soup.find('article') or soup.find('div', class_=re.compile(r'article|content|body'))
    if not article:
        article = soup.body
...
def fetch_and_convert():
    ...
    for a in soup.find_all('a', href=True):
        href = a['href']
        if '/zh/' in href or href.startswith('/zh/'):
    ...
    xml += '  <title>德国之声中文网 RSS（简体/全文）</title>\n'
    xml += '  <link>https://www.dw.com/zh/</link>\n'
    xml += '  <description>德国之声中文网全文抓取、繁转简</description>\n'
